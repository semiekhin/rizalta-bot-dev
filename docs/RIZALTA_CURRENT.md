...
## ✅ Что сделано 19.03.2026 — Сводное КП для лотов К3
- Сводное КП: 3 лота (В713, В715, В721) из building=3
- Лоты из `data/corp3_units.json` → `properties.db` (building=3), пересчёт: площадь × цена_за_м²
- Планировки из `data/corp3_layouts/` — сжатие Pillow (800×1130, quality=75), base64 inline
- 3 индивидуальных КП PDF + сводная таблица (landscape A4, wkhtmltopdf)
- Сводная: 2 таблицы (рассрочка 12 мес 0% + 18 мес), три цены (общая / скидка 5% / 100% оплата), ПВ 30/40/50%, строка ИТОГО
- Склейка через PyPDF2
- Временные лоты удалены из properties.db после генерации

## ✅ Что сделано 24.03.2026 — Cleanup
- `kp_pdf_generator.py`: убран А101 из CUSTOM_INSTALLMENT_UNITS
- `kp_pdf_generator.py`: `download_layout()` поддержка `data:image` base64 (полезный фикс для К3 планировок)
- `kp_pdf_generator.py`: вернул ширину планировки 380px (220px было временно для сводного КП)
- `units_db.py`: откат убранного фильтра `status='available'` в `get_lot_by_code()` — штатный режим
- **Коммиты:** `9811b2e`, `6ad490c`
...
## [WebApp] Сессия 11.03.2026 — v0.9.7

**Версия:** v0.9.7 (Инвестиционная сводка по лоту)

**Что сделано:**
- Модалка "📊 Инвестиционная сводка" в LotDetail.jsx — агрегирует данные из 5 API (Promise.all)
- lot_summary_pdf_generator.py — PDF в dark RIZALTA branding (#263524 / #D4A84B)
- POST /api/lot-summary-pdf — скачивание PDF с кириллическим именем (RFC 5987)
- Chat.jsx cleanup — удалены кнопки "Фин. отчёт по лоту" и "Портфель по бюджету" (-96 строк)
- Портфельный калькулятор (отдельный экран) деприоритизирован

**Бэклог:**
- 🔴 Траншевая ипотека (tranche mortgage calculator)
- 🔴 PDF инвестиционного отчёта из чата
- 🟡 Портфельный калькулятор (деприоритизирован, менеджеры предпочитают вручную)
