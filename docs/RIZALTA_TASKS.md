...
## Бот бэклог (04.03.2026)
### 🔴 Ближайшие
1. **Траншевая ипотека: починить планировку в PDF** — Pillow CMYK→RGB работает, но изображение не попадает в HTML/PDF
2. **Админ-панель** — hiding/showing buildings, whitelist management

### 🟡 Средний приоритет
3. К4 whitelist (инфраструктура готова от К3)
4. Деплой траншевой ипотеки в PROD (после фикса планировки)

### 05.03.2026: PDF планировка траншевой ипотеки (продолжение)
- Попытки: CSS filter (игнорируется wkhtmltopdf), base64→file://, --disable-smart-shrinking
- Планировка в PDF до сих пор не отображается — диагностика не завершена
- Версия: 2.7.2

### 05.03.2026 part 2: Деплой траншевой ипотеки + фикс PDF (ЗАВЕРШЕНО)
...
- ✅ "Ипотека" → "Ипотека СОВКОМБАНК 4.4%" в DEV и PROD kp.py

## Бот бэклог (05.03.2026, обновлено)
### 🔴 Ближайшие
1. **Админ-панель** — hiding/showing buildings, whitelist management

### 🟡 Средний приоритет
2. К4 whitelist (инфраструктура готова от К3)

### 🟢 Nice-to-have
3. README для handlers/, services/


---

## [WebApp] Актуальный бэклог (11.03.2026, v0.9.7)

### 🔴 Ближайшие
- Траншевая ипотека: новая карточка аналогично Совкомбанк, коэффициенты: 0.01843@21.7%, 0.01800@21.2%, 0.01636@19.2%
- PDF инвест-отчёта из чата: кнопка "Скачать PDF" после генерации

### 🟡 Деприоритизировано
- Портфельный калькулятор (отдельный экран) — менеджеры предпочитают вручную

### ✅ Выполнено в v0.9.7
- Инвестиционная сводка по лоту (modal + PDF)
- Chat.jsx cleanup (-96 строк)

## WebApp задачи (обновлено 16.03.2026)
- ✅ Траншевая ипотека (калькулятор + PDF + UI)
- ✅ YandexGPT интеграция (свободный чат)
- ✅ Редизайн меню лота
