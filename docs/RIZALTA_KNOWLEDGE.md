...
### units_db.py: обязательный фильтр status='available'

**Проблема:** Парсер rclick делает DELETE+INSERT, поэтому раньше все записи были available. При ручном изменении статуса (sold) лоты всё равно показывались — 8 из 9 функций не фильтровали по status.

**Решение (v2.7.1):** Добавлен AND status='available' во ВСЕ SELECT-запросы units_db.py (9 штук).

**Важно:** Если нужно скрыть лот — можно менять status на 'sold'. Но при следующем запуске парсера он перезапишет все записи.

### Custom Installment + building

**Список:** В327, В615, В527, В517, В617, В525, В625, А101

**Важно:** Проверка привязана к building==1. Коды могут совпадать между корпусами (В327 есть в К1 и К3), поэтому ОБЯЗАТЕЛЬНА проверка:
```python
is_custom = lot['code'] in CUSTOM_INSTALLMENT_UNITS and lot.get('building') == 1
```

3 места: kp_pdf_generator.py (строка ~231), kp.py (строки ~230, ~733)
